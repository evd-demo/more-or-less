...
class DenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=12, block_config=(16, 16, 16), compression=0.5,
                 ...
                 num_classes=10, small_inputs=True, efficient=False, grayscale=False):
        ...
        # if small_inputs:
        if grayscale:
            # input channels have size 1 as it is gray scaled
            self.features = nn.Sequential(OrderedDict([
                ('conv0', nn.Conv2d(1, num_init_features, kernel_size=3, stride=1, padding=1, bias=False)),
            ])) # Kernel size for first layer is 3 and the number of filters is 24
        else:
            self.features = nn.Sequential(OrderedDict([
        ...
        # Linear layer (adjust based on the fact that that average pooling is going to produce larger output 
        # when small_input is false)
        # print(num_features)
        if small_inputs:
